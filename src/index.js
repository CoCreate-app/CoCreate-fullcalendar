...
function setData(element, data) {
	...
	for (let object of data.objects) {
		...
		if (event) {
			...
			if (object.displayName) event.setProp("title", object.displayName);

			if (object.allDay) event.setAllDay(object.allDay);

			if (object.startDate || object.startTime) {
				let startDate, startTime;
				if (!object.startDate) startDate = getDateString(event.start);
				if (!object.startTime) startTime = getTimeString(event.start);
				event.setStart(startDate + "T" + startTime);
			}

			if (object.endDate || object.endTime) {
				let endDate, endTime;
				if (!object.startDate) startDate = getDateString(event.end);
				if (!object.startTime) endTime = getTimeString(event.end);

				endDate = convertEndDateForRender(
					endDate,
					endTime,
					event.allDay
				);
			...
			event.end = convertEndDateForRender(
				object.endDate,
				object.endTime,
				object.allDay
			);
			...
			if (object.startTime) event.start += "T" + object.startTime;
			if (object.endTime) event.end += "T" + object.endTime;
			if (object.startDate && object.endDate) eventSource.push(event);
		}
	}
...
function initButton(element) {
	if (element && !Array.isArray(element)) element = [element];
	else if (!element) element = document.querySelectorAll("[calendar-view]");
...
function calendarBtnClicked(calendar, type) {
	for (let i = 0; i < calendar.length; i++) {
		...
		if (type === "today") calendar[i].today();
		else calendar[i].changeView(type);
	}
}
